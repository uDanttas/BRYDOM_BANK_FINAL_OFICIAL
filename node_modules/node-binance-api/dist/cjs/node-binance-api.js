...
*/
    ...
    * @return {promise or undefined} - omitting the callback returns a promise
    */
    async cancelOrders(symbol, params = {}) {
    ...
    */
     ...
     * @return {undefined}
     */
    handleSocketClose(reconnect, code, reason) {
        ...
        if (this.Options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && this.endpoint.length === 60)
                this.Options.log('Account data WebSocket reconnecting...');
            else
                this.Options.log('WebSocket reconnecting: ' + this.endpoint + '...');
            try {
                reconnect();
            }
            catch (error) {
    ...
    handleFuturesSocketClose(reconnect, code, reason) {
        ...
        if (this.Options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && this.endpoint.length === 60)
                ...
                this.Options.log('Futures WebSocket reconnecting: ' + this.endpoint + '...');
            try {
                reconnect();
            }
            catch (error) {
    ...
    handleDeliverySocketClose(reconnect, code, reason) {
        ...
        if (this.Options.reconnect && this.reconnect && reconnect) {
            if (this.endpoint && this.endpoint.length === 60)
                ...
                this.Options.log('Delivery WebSocket reconnecting: ' + this.endpoint + '...');
            try {
                reconnect();
            }
            catch (error) {
 ...
 */
    ...
    futuresCandlesticks(symbols, interval, callback) {
        const reconnect = () => {
            if (this.Options.reconnect)
                this.futuresCandlesticks(symbols, interval, callback);
        };
        let subscription;
        if (Array.isArray(symbols)) {
            if (!this.isArrayUnique(symbols))
